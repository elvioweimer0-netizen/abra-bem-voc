...
function TodaySection(props: { meetings: Meeting[]; dailyMeeting?: Meeting; dailyMinute?: MeetingMinute; isRecording: boolean; joiningDaily: boolean; uploading: boolean; processingRecording: boolean; occurrences: Occurrence[]; notices: Notice[]; sale: string; setSale: (value: string) => void; goal: string; setGoal: (value: string) => void; decisions: string; setDecisions: (value: string) => void; acceptedSuggestions: PautaSuggestion[]; profiles: Map<string, ProfileLite>; onJoin: () => void; onUpload: (event: React.ChangeEvent<HTMLInputElement>) => void; onCloseDaily: () => void; onCloseMeeting: (meeting?: Meeting) => void }) {
  const otherMeetings = props.meetings.filter((meeting) => meeting.id !== props.dailyMeeting?.id);
  return <div className="space-y-6"><Card className="rounded-[1.25rem] border-primary/30 bg-primary/10 shadow-lg"><CardContent className="p-6"><div className="flex items-start justify-between gap-3"><div><h2 className="text-2xl font-bold text-foreground">Reunião imediata</h2><p className="mt-1 text-sm text-muted-foreground">Diária operacional · 09:30</p></div><Mic className="h-8 w-8 text-primary" /></div>{props.isRecording && <Badge className="mt-4 bg-destructive text-destructive-foreground">🔴 Gravando</Badge>}<Button className="mt-5 min-h-14 w-full rounded-2xl gap-2 shadow-lg" onClick={props.onJoin} disabled={props.joiningDaily}><Mic className="h-5 w-5" /> {props.joiningDaily ? "Iniciando..." : "Entrar/Iniciar"}</Button><Input id="manual-recording" type="file" accept="audio/*,video/*" className="hidden" onChange={props.onUpload} disabled={props.uploading || !props.dailyMeeting} /><Button variant="outline" className="mt-3 min-h-12 w-full rounded-2xl gap-2 bg-card" asChild disabled={props.uploading || !props.dailyMeeting}><label htmlFor="manual-recording"><Upload className="h-5 w-5" /> {props.uploading ? "Processando..." : "Subir gravação manual"}</label></Button></CardContent></Card>{props.dailyMinute && <MinutesCard minute={props.dailyMinute} />}<Card className="rounded-[1.25rem] shadow-lg"><CardHeader><CardTitle>Pauta automática</CardTitle></CardHeader><CardContent className="space-y-5"><AgendaBlock title="1. B.O. do dia anterior" items={props.occurrences.map((o) => o.descricao)} empty="Sem B.O.s médios/altos ontem." /><AgendaBlock title="2. Informativos novos" items={props.notices.map((n) => n.titulo)} empty="Sem avisos nas últimas 24h." />{props.acceptedSuggestions.length > 0 && <div className="rounded-2xl border border-primary/20 bg-primary/10 p-4"><p className="font-bold text-foreground">📌 Sugestões da liderança</p>{props.acceptedSuggestions.map((suggestion) => <p key={suggestion.id} className="mt-2 text-sm text-muted-foreground">• {suggestion.title} — {props.profiles.get(suggestion.suggested_by)?.nome || "Gerente"}</p>)}</div>}<Input placeholder="3. Venda do dia anterior por loja" value={props.sale} onChange={(e) => props.setSale(e.target.value)} /><Input placeholder="4. Metas do dia por loja" value={props.goal} onChange={(e) => props.setGoal(e.target.value)} /><Textarea placeholder="Decisões tomadas" value={props.decisions} onChange={(e) => props.setDecisions(e.target.value)} /><Button variant="outline" className="min-h-12 w-full rounded-2xl gap-2" onClick={props.onCloseDaily} disabled={props.processingRecording}><FileText className="h-5 w-5" /> {props.processingRecording ? "Processando..." : "Encerrar reunião e gerar ATA"}</Button></CardContent></Card><div className="grid gap-4 sm:grid-cols-2">{otherMeetings.map((meeting) => <Card key={meeting.id} className="rounded-[1.25rem] shadow-lg transition-all hover:scale-[1.02] hover:shadow-xl"><CardContent className="p-5"><Badge variant="secondary">{formatMeetingType(meeting.type)}</Badge><h3 className="mt-3 text-lg font-bold text-foreground">{meeting.title}</h3><p className="text-sm text-muted-foreground">{meeting.scheduled_time.slice(0, 5)} · {meeting.is_monthly_in_person ? "Presencial" : "Online"}</p><Button className="mt-4 w-full rounded-2xl" onClick={() => props.onCloseMeeting(meeting)}>Gerar ATA</Button></CardContent></Card>)}</div></div>;
}
...
function minuteStatus(minute?: MeetingMinute) {
  ...
  if (minute.processing_status === "failed") return { label: "❌ Erro", tone: "destructive" as const };
...
function HistoryMeetingCard({ meeting, minute, attendees, pendingSuggestions, onOpen, onRefresh, onRetry, retrying }: { meeting: Meeting; minute?: MeetingMinute; attendees: MeetingAttendee[]; pendingSuggestions: number; onOpen: () => void; onRefresh: () => void; onRetry: (minute: MeetingMinute) => void; retrying: boolean }) {
  ...
  return <Card className="rounded-[1.25rem] shadow-lg transition-all duration-200 hover:scale-[1.02] hover:shadow-xl"><CardContent className="p-5"><button type="button" className="w-full text-left" onClick={onOpen}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-sm text-muted-foreground">📅 {meetingDate(meeting).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" })}</p><h3 className="mt-2 text-lg font-bold text-foreground">{minute?.titulo || meeting.title}</h3><p className="mt-1 text-sm text-muted-foreground">🏷️ {formatMeetingType(meeting.type)} · ⏱️ {formatDuration(meeting)}</p><div className="mt-3 flex items-center gap-1">{(attendees.length ? attendees : [{ id: "fallback", role_label: "Participante" } as MeetingAttendee]).slice(0, 5).map((attendee, index) => <div key={attendee.id} className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-card bg-accent text-xs font-bold text-accent-foreground" style={{ marginLeft: index ? -8 : 0 }}>{firstName(attendee.role_label).slice(0, 1)}</div>)}</div></div><div className="flex shrink-0 flex-col items-end gap-2"><Badge variant={status.tone}>{status.label}</Badge><Badge className={sentimentTone}>{sentiment === "positivo" ? "🙂" : sentiment === "tenso" ? "😟" : "😐"} {sentiment}</Badge>{pendingSuggestions > 0 && <Badge variant="destructive">🤖 {pendingSuggestions}</Badge>}</div></div></button>{(!minute || minute.processing_status === "pending" || minute.processing_status === "processing" || minute.processing_status === "failed") && <Button variant="outline" className="mt-4 min-h-11 w-full rounded-2xl gap-2" onClick={minute?.processing_status === "failed" && minute ? () => onRetry(minute) : onRefresh} disabled={retrying}><RefreshCw className="h-4 w-4" /> {minute?.processing_status === "failed" ? (retrying ? "Tentando..." : "Tentar novamente") : "Atualizar"}</Button>}</CardContent></Card>;
}
