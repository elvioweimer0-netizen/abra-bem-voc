...
export default function ReunioesLideranca() {
  ...
  const closeMeeting = async (meeting?: Meeting) => {
    if (!meeting) return;
    const text = [`ATA - ${meeting.title}`, `Participante: ${profile?.nome || "Usuário"}`, `Ocorrências: ${occurrences.map((o) => o.descricao).join("; ") || "sem registros críticos"}`, `Informativos: ${notices.map((n) => n.titulo).join("; ") || "sem novos avisos"}`, `Venda anterior: ${sale || "não informada"}`, `Metas: ${goal || "não informadas"}`, `Decisões: ${decisions || "sem decisões registradas"}`].join("\n");
