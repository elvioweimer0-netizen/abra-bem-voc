...
export default function VisaoGeralAdmin() {
  ...
  useEffect(() => { const load = async () => { const today = todayISO(); const [{ data: unitData }, { data: completionData }, { data: boData }, { data: inspectionData }, { data: managerData }] = await Promise.all([db.from("units").select("id, code, name").eq("active", true).order("code"), db.from("checklist_completions").select("unit_id, status").eq("data", today), db.from("leadership_occurrences").select("id, unit_id, status, gravidade, descricao").neq("status", "resolvido"), db.from("leadership_inspections").select("id, unit_id, score_organizacao, score_atendimento, score_estoque, score_limpeza").eq("data", today), db.from("profiles").select("user_id, nome, unit_id").in("cargo", ["gerente", "gerente_loja", "encarregado", "lider"])]); setUnits(unitData || []); setCompletions(completionData || []); setBos(boData || []); setInspections(inspectionData || []); setManagers(managerData || []); }; load(); }, []);
  ...
  return <div className="space-y-5"><section className="rounded-xl bg-primary p-4 text-primary-foreground shadow-sm"><p className="text-sm opacity-90">Visão Geral</p><h1 className="text-2xl font-bold">Painel do Admin</h1></section>
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><Card><CardContent className="p-4"><p className="text-sm text-muted-foreground">Cumprimento médio</p><p className="text-3xl font-bold text-foreground">{avg}%</p></CardContent></Card><Card><CardContent className="p-4"><p className="text-sm text-muted-foreground">B.O.s abertos</p><p className="text-3xl font-bold text-foreground">{bos.length}</p></CardContent></Card><Card><CardContent className="p-4"><p className="text-sm text-muted-foreground">Inspeções hoje</p><p className="text-3xl font-bold text-foreground">{inspections.length}</p></CardContent></Card></div>
