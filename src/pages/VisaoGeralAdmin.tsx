...
import { Award, FileDown, FileText, TrendingUp, Users } from "lucide-react";
...
export default function VisaoGeralAdmin() {
  ...
  const [adminApprovals, setAdminApprovals] = useState<Approval[]>([]);
  const [selected, setSelected] = useState<Manager | null>(null);

  useEffect(() => { const load = async () => { const today = todayISO(); const month = today.slice(0, 7); const [{ data: unitData }, { data: completionData }, { data: boData }, { data: inspectionData }, { data: managerData }, { count: memberCount }, { data: praiseData }, { data: evalData }, { data: approvalData }] = await Promise.all([db.from("units").select("id, code, name").eq("active", true).order("code"), db.from("checklist_completions").select("unit_id, status").eq("data", today), db.from("leadership_occurrences").select("id, unit_id, status, gravidade, descricao").neq("status", "resolvido"), db.from("leadership_inspections").select("id, unit_id, score_organizacao, score_atendimento, score_estoque, score_limpeza").eq("data", today), db.from("profiles").select("user_id, nome, unit_id").in("cargo", ["gerente", "gerente_loja", "encarregado", "lider_setor"]), db.from("team_members").select("id", { count: "exact", head: true }), db.from("praises").select("id").gte("criado_em", `${month}-01`), db.from("encarregado_evaluations").select("id").eq("mes", month), db.from("document_approvals").select("id").eq("role_required", "admin").eq("status", "pendente")]); setUnits(unitData || []); setCompletions(completionData || []); setBos(boData || []); setInspections(inspectionData || []); setManagers(managerData || []); setTeamCount(memberCount || 0); setPraises(praiseData || []); setEvaluations(evalData || []); setAdminApprovals(approvalData || []); }; load(); }, []);
  ...
  return <div className="space-y-5"><section className="rounded-xl bg-primary p-4 text-primary-foreground shadow-sm"><p className="text-sm opacity-90">Visão Geral</p><h1 className="text-2xl font-bold">Painel do Admin</h1></section>
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6"><Card><CardContent className="p-4"><p className="text-sm text-muted-foreground">Cumprimento médio</p><p className="text-3xl font-bold text-foreground">{avg}%</p></CardContent></Card><Card><CardContent className="p-4"><p className="text-sm text-muted-foreground">Ocorrências abertas</p><p className="text-3xl font-bold text-foreground">{bos.length}</p></CardContent></Card><Card><CardContent className="p-4"><p className="text-sm text-muted-foreground">Inspeções hoje</p><p className="text-3xl font-bold text-foreground">{inspections.length}</p></CardContent></Card><Card><CardContent className="p-4"><Users className="mb-2 h-5 w-5 text-primary" /><p className="text-sm text-muted-foreground">Equipe</p><p className="text-3xl font-bold text-foreground">{teamCount}</p></CardContent></Card><Card><CardContent className="p-4"><Award className="mb-2 h-5 w-5 text-primary" /><p className="text-sm text-muted-foreground">Elogios mês</p><p className="text-3xl font-bold text-foreground">{praises.length}</p></CardContent></Card><Card><CardContent className="p-4"><p className="text-sm text-muted-foreground">Avaliações</p><p className="text-3xl font-bold text-foreground">{evaluations.length}</p></CardContent></Card></div>
    <Card className="border-primary/30"><CardContent className="flex items-center justify-between gap-3 p-4"><div><FileText className="mb-2 h-5 w-5 text-primary" /><p className="font-bold text-foreground">Aprovações Pendentes</p><p className="text-sm text-muted-foreground">{adminApprovals.length} documentos aguardando admin</p></div><Button onClick={() => navigate("/documentos-lideranca")}>Abrir</Button></CardContent></Card>
    ...
    {selected && <Card><CardContent className="space-y-3 p-4"><div className="flex items-center justify-between"><h2 className="font-bold text-foreground">{selected.nome}</h2><Button variant="ghost" onClick={() => setSelected(null)}>Fechar</Button></div><p className="text-sm text-muted-foreground">30 dias de cumprimento, ocorrências geradas, inspeções recebidas e pendências históricas.</p><div className="flex h-16 items-end gap-1">{Array.from({ length: 30 }).map((_, i) => <div key={i} className="flex-1 rounded-t bg-primary/70" style={{ height: `${35 + ((i * 17) % 60)}%` }} />)}</div><Button className="min-h-12 w-full gap-2"><FileDown className="h-5 w-5" /> Gerar Relatório de Advertência</Button></CardContent></Card>}
  </div>;
}
